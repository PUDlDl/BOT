...
const config = {
    A187: 'SIMSALABIMBOT', // TOBAT SU ASU
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, AnxietyBot`)
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝘀𝗮𝘆𝗼𝗻𝗮𝗿𝗮𝗮𝗮🥳 @${num.split('@')[0]} 𝗴𝗮 𝘂𝘀𝗮𝗵 𝗯𝗮𝗹𝗶𝗸 𝗸𝗲𝘀𝗶𝗻𝗶 𝗹𝗮𝗴𝗶 𝘆𝗮𝗵 𝘁𝗼𝗱`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '❬❗❭ 𝗕𝗢𝗧 𝗛𝗔𝗥𝗨𝗦 𝗝𝗔𝗗𝗜 𝗔𝗗𝗠𝗜𝗡 𝗗𝗨𝗟𝗨'
				}
			}
            ...
            client.sendFile(from, sesPic, 'session.png', 'ngengg ^_^...', id)
            break	
				case 'gtts':	
				case 'tts':
			...
			case 'loli': 
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('𝘁𝗲𝗿𝘂𝘀 𝗮𝗸𝘂 𝘀𝘂𝗿𝘂𝗵 𝗻𝘂𝗹𝗶𝘀 𝗮𝗽𝗮? 𝗻𝘂𝗹𝗶𝘀 𝘁𝘆𝘁𝗱 𝗴𝗶𝘁𝘂?')
					teks = body.slice(7)
					reply(mess.wait)
