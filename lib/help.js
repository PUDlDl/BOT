
const help = (prefix) => { 
	return `Haaeeeyyyy👋🏻                 

Diawali dengan menggunakan command/perintah dibawah ini.
Prefix bisa berganti kapanpun oleh ownerbot.

ꔷ Menu - menu AnxietyBot *v 3.2*

╭┈─☆───────────
〲ꔷ❥ௐ㆒ *CREATE MAKER*
〲ꔷ❥ௐ㆒ *MEDIA*
〲ꔷ❥ௐ㆒ *DOWNLOADER*
〲ꔷ❥ௐ㆒ *OWNER*
〲ꔷ❥ௐ㆒ *GROUP*
〲ꔷ❥ௐ㆒ *OTHER*
╰┅┄☆┅┅┅┅┅┅┅┅┅┅─

  ╔─━━━━━━░★░━━━━━━─╗         
┏━━°✪〘 INFORMATION 〙✪°━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃
┣ௐ *${prefix}info*
┣ௐ *${prefix}owner*
┣ௐ *${prefix}donasi*
┣ௐ *${prefix}help/menu*
┃
┃ ╔─━━━━━━░★░━━━━━━─╗         
┏━°✪〘 CREATE MAKER 〙✪°━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃
┣ௐ *${prefix}ttp*
┣ௐ *${prefix}epep*
┣ௐ *${prefix}nulis*
┣ௐ *${prefix}rtext*
┣ௐ *${prefix}water*
┣ௐ *${prefix}tahta*
┣ௐ *${prefix}sticker*
┣ௐ *${prefix}tsticker*
┣ௐ *${prefix}quotemaker*
┣ௐ *${prefix}ninjalogo*
┣ௐ *${prefix}lionlogo*
┣ௐ *${prefix}lovemaker*
┃
┃ ╔─━━━━━━░★░━━━━━━─╗         
┏━━━━°✪〘 MEDIA 〙✪°━━━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃
┣ௐ *${prefix}ocr*
┣ௐ *${prefix}tts*
┣ௐ *${prefix}meme*
┣ௐ *${prefix}loli*
┣ௐ *${prefix}tiktok*
┣ௐ *${prefix}nsfwloli*
┣ௐ *${prefix}memeindo*
┃
┃ ╔─━━━━━━░★░━━━━━━─╗             
┏━━°✪〘 DOWNLOADER 〙✪°━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃
┣ௐ *yt* [link]
┣ௐ *play* [judul lagu]
┃
┃ ╔─━━━━━━░★░━━━━━━─╗         
┏━━━━°✪〘 GROUP 〙✪°━━━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃ ╔─┄─┄─┄─┄─┄┄┄┄┄─┄࿐
┃ ╚─┈─➢ _AdminOnly_
┃
┣ௐ *${prefix}tag*
┣ௐ *${prefix}join*
┣ௐ *${prefix}tagme*
┣ௐ *${prefix}add* [62xxx]
┣ௐ *${prefix}kick* [tag]
┣ௐ *${prefix}setpp*
┣ௐ *${prefix}demote* [tag]
┣ௐ *${prefix}promote* [tag]
┣ௐ *${prefix}setpp*
┣ௐ *${prefix}group* [buka/tutup]
┣ௐ *${prefix}welcome* [1/0]
┣ௐ *${prefix}nsfw* [1/0]
┣ௐ *${prefix}simih* [1/0]
┣ௐ *${prefix}tagall*
┣ௐ *${prefix}mentionall*
┣ௐ *${prefix}everyone*
┃
┃ ╔─━━━━━━░★░━━━━━━─╗         
┏━━━━°✪〘 OWNER 〙✪°━━━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃ ╔─┄─┄─┄─┄─┄┄┄┄┄┄─┄࿐
┃ ╚─┈─➢ _OwnerBot_
┃
┣ௐ *${prefix}bc* [live]
┣ௐ *${prefix}clone* [tag]
┣ௐ *${prefix}block* [tag]
┣ௐ *${prefix}leave* [-]
┣ௐ *${prefix}unblock* [tag]
┣ௐ *${prefix}clearall* [x]
┣ௐ *${prefix}setprefix* [...]
┃
┃ ╔─━━━━━━░★░━━━━━━─╗         
┏━━━━°✪〘 OTHER 〙✪°━━━━
┃ ╚─━━━━━━░★░━━━━━━─╝
┃
┣ௐ *${prefix}say*
┣ௐ *${prefix}simi*
┣ௐ *${prefix}wait*
┣ௐ *${prefix}party*
┣ௐ *${prefix}getses*
┣ௐ *${prefix}bucin*
┣ௐ *${prefix}fitnah*
┣ௐ *${prefix}thunder*
┣ௐ *${prefix}ur12img*
┣ௐ *${prefix}ceksider*
┣ௐ *${prefix}ytsearch*
┣ௐ *${prefix}listadmin*
┣ௐ *${prefix}persengay*
┣ௐ *${prefix}tiktokstalk*
┣ௐ *${prefix}hidetag*
┣ௐ *${prefix}ramaljadian*
┣ௐ *${prefix}tod*
┣ௐ *${prefix}truth*
┣ௐ *${prefix}dare*
┣ௐ *${prefix}qrcode*
┃
┣━━━━━━━━━━━━━━━━━━━━
┃╔─┄─┄┄┄┄┄┄─┄─┄─┄─┄࿐
┃╚─┈─➢ *_AnxietyBot_*
┗━━━━━━━━━━━━━━━━━━━━
┃
╔─━━━━━━░★░━━━━━━─╗         
ꔷ㆒  °✪〘 LAST 〙✪°  ㆒ꔷ
╚─━━━━━━░★░━━━━━━─╝
┃
╔════════════════════━───¤͜͡҈➢
║ *SEKEDAR INFO*
╠════════════════════
║├ௐ *CREATOR*
║├ௐ HumanAnxiety
╠════════════════════
║¤͜͡҈¤͜͡҈¤͜͡҈¤͜͡҈¤͜͡҈➥ *ABOUT*
╠════════════════════
║BOT INI MENGGUNAKAN SOURCE 
║CODE YANG SAYA DAPATKAN DAN
║SAYA MODIFIKASI DENGAN
║PENAMBAHAN, PENGURANGAN, 
║DAN PERUBAHAN BEBERAPA FITUR.
║────────────────────────
║BOT INI DIBUAT MENGGUNAKAN
║NODE.JS & JavaScript
║────────────────────────
║BOT ON 24 JAM
║KALO OFF BERARTI SIBUK (>.<)
║────────────────────────
║HANYA BOT TERMUX YG
║SEDERHANA JADI JAN 
║BERHARAP LEBIH ツ
║────────────────────────
║*JIKA ADA YG ERORR HUB ADMIN*
╠════════════════════
║ *RULE§ BOT*
╠════════════════════
║╭────❉ *RULE§ BOT* ❉──
║│CALL/VC = BLOK 24 JAM
║│SPAM CALL/VC = PERMANENT
║│NO SHARE VIRৡৢ¤͜͡҈!!
║│JAN ASAL MASUKIN KE GC LU
║╰───────────────────
╠════════════════════
║ *THANKS TO* 🤝🏻
╠════════════════════
║├≽️Allah
║├≽️Orang Tua
║├≽️Keluarga
║├≽️Kamu yg baca
║├≽️Teman
║├≽️Teman Lupdir
║├≽️Teman yg dtg pas ada maunya
║├≽️Sahabat
║├≽️Kerabat
║├≽️Saudara
║├≽️Musuh
║├≽️Pacar
║├≽️Stm
║├≽️Ttm
║├≽️Selingkuhan
║├≽️Mantan
║├≽️Subscriber gw
║├≽️Followers gw
║├≽️Viewers gw
║├≽️Yg sekontak wa gw
║├≽️Langit
║├≽️Bintang
║├≽️Galaksi
║├≽️Matahari
║├≽️Bulan
║├≽️Merkurius
║├≽️Venus
║├≽️Mars
║├≽️Jupiter
║├≽️Saturnus
║├≽️Uranus
║├≽️Neptunus
║├≽️Dan planet lain
║├≽️Terutama bumi
║├≽️Angin
║├≽️Udara
║├≽️Atsmosfer
║├≽️Langit
║├≽️Awan
║├≽️Hujan
║├≽️Pohon
║├≽️Oksigen
║├≽️Tumbuhan
║├≽️Hewan
║├≽️Buah
║├≽️Kucing gw
║├≽️Ikan
║├≽️Ayam
║├≽️Tanah
║├≽️Air
║├≽️Api
║├≽️Listrik
║├≽️Pln
║├≽️Indihome
║├≽️Rumah gw
║├≽️Dll Selengkapnya yap
║├≽️Cape gw ngetiknya anjrott
║├≽️dahbye
╠════════════════════
║ ꔷ㆒ *_AnxietyBot_*
╚════════════════════━───¤͜͡҈➢`
}
exports.help = help



  
